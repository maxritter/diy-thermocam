...
# Version definition
version = "Version 2.02 from 06.02.2017"
...
def getFrame():
    ...
    # Save a frame to the internal card of the Thermocam
    elif cmd == FRAME_CAPTURE:
        # Display save message
        displayText("Saving to card..", False)

        # Capture and check for success
        if sendCapture():
            displayText("Frame saved!", True)
        else:
            displayText("Error saving frame!", True)
...
def sendCapture():
    ...
    try:
        # Send start command
        ser.write(chr(CMD_FRAME_CAPTURE))
        # Read answer
        inData = map(ord, ser.read(1))
    ...
    # Set the timeout back to 3 seconds
    ser.timeout = 3.0
