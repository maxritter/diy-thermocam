...
def sendCommandWithFloat(cmd, data):
    ...
    try:
        ...
        sendArray = (chr(cmd).encode() + struct.pack('<f', data))
        ser.write(sendArray)
        # Get ACK
        inData = list(ser.read(1))
...
def setColorScheme(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_colorscheme).encode(), chr(colorScheme).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setSpot(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_showspot).encode(), chr(spotEnabled).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setBar(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_showcolorbar).encode(), chr(barEnabled).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setMinMax(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_showminmax).encode(), chr(minMaxEnabled).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setTextColor(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_textcolor).encode(), chr(textColor).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setFilterType(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_filtertype).encode(), chr(filterType).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setLimits(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_limits).encode(), chr(adjustLimits).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setFormat(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_tempformat).encode(), chr(tempFormat).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
    ...
    # Update UI
    if tempFormat:
        buttonFormat.caption = "Format: " "\N{DEGREE SIGN}" + "F"
    else:
        buttonFormat.caption = "Format: " "\N{DEGREE SIGN}" + "C"
...
def setRotation(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_rotation).encode(), chr(rotateScreen).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def setLaser(updateValue):
    ...
    if updateValue:
        ...
        try:
            # Send command and new value
            sendArray = (chr(cmd_set_laser).encode(), chr(laserEnabled).encode())
            ser.write(sendArray)
            # Get ACK
            inData = list(ser.read(1))
...
def runShutter():
    ...
    try:
        # Send command
        ser.write(chr(cmd_set_shutterrun).encode())
        # Get ACK
        inData = list(ser.read(1))
...
def runCalibration():
    ...
    try:
        # Send command
        ser.write(chr(cmd_set_calibration).encode())
        # Get ACK
        inData = list(ser.read(1))
...
def getCalibration():
    ...
    try:
        # Send command
        ser.write(chr(cmd_get_calibdata).encode())
        # Get data
        inData = list(ser.read(8))
...
def drawButtons():
    ...
    buttonFormat = pygbutton.PygButton((482, 565, 147, 27), 'Format: ' '\N{DEGREE SIGN}' + 'F')
...
def saveVisual():
    ...
    try:
        ...
        sendArray = (chr(cmd_get_visualimg).encode(), chr(1).encode())
        ser.write(sendArray)
        # Get visual image length
        inData = list(ser.read(2))
...
def tempFunc(mousePos, add):
    ...
    try:
        # Build send array
        sendArray = (chr(cmd_set_temppoints).encode(),)
        for i in range(0, 96):
            # Index
            sendArray += (chr((tempPoints[i][0] & 0xFF00) >> 8).encode(),)
            sendArray += (chr(tempPoints[i][0] & 0x00FF).encode(),)
            # Value
            sendArray += (chr((tempPoints[i][1] & 0xFF00) >> 8).encode(),)
            sendArray += (chr(tempPoints[i][1] & 0x00FF).encode(),)
...
def getFrameData():
    ...
    try:
        leptonData = list(ser.read(length))
    ...
    try:
        additionalData = list(ser.read(16))
    ...
    spotTemp = 0.00
    #spotTemp = float(struct.unpack('f', bytes(spotTemp, encoding='utf8'))[0])

    # Read calibration offset
    calOffset = [additionalData[8], additionalData[9], additionalData[10], additionalData[11]]
    calOffset = ''.join(chr(i) for i in calOffset)
    calOffset = -273.15
    #calOffset = float(struct.unpack('f', bytes(calOffset, encoding='utf8'))[0])

    # Read calibration slope
    calSlope = [additionalData[12], additionalData[13], additionalData[14], additionalData[15]]
    calSlope = ''.join(chr(i) for i in calSlope)
    calSlope = 0.1
    #calSlope = float(struct.unpack('f', bytes(calSlope, encoding='utf8'))[0])
...
def getConfigData():
    ...
    try:
        ...
        ser.write(chr(cmd_get_configdata).encode())
        # get data
        configData = list(ser.read(10))
...
def serialHandler():
    ...
    try:
        ...
        if renderMode:
            ser.write(chr(cmd_rawframe).encode())
        # Send a get display frame command
        else:
            ser.write(chr(cmd_displayframe).encode())
...
def getTempPoints():
    ...
    try:
        ...
        ser.write(chr(cmd_get_temppoints).encode())
        # Wait for response
        inData = list(ser.read(384))
...
def setTime():
    ...
    try:
        # Send set time command
        ser.write(chr(cmd_set_time).encode())
        # Set time as bytestream
        ser.write(bytes(timeOut, encoding='utf8'))
        # Wait for ACK
        inData = list(ser.read(1))
...
def getHWVersion():
    ...
    try:
        ...
        ser.write(chr(cmd_get_hwversion).encode())
        # Receive answer
        inData = list(ser.read(1))
...
def getFWVersion():
    ...
    try:
        ...
        ser.write(chr(cmd_get_fwversion).encode())
        # Receive answer
        inData = list(ser.read(1))
...
def getHQResolution():
    ...
    try:
        # Send get hardware version command
        ser.write(chr(cmd_get_hqresolution).encode())
        # Receive answer
        inData = list(ser.read(1))
...
def getCalStatus():
    ...
    try:
        # Send get hardware version command
        ser.write(chr(cmd_get_calstatus).encode())
        # Receive answer
        inData = list(ser.read(2))
...
def getDiagnostic():
    ...
    try:
        ...
        ser.write(chr(cmd_get_diagnostic).encode())
        # Receive answer
        inData = list(ser.read(1))
...
def sendStart():
    ...
    try:
        # Send start command
        ser.write(chr(cmd_start).encode())
        # Read answer
        inData = list(ser.read(1))
